...
- `...Installer.exe` installs beside official OBS in
  `C:\Program Files\obs-studio-capture-stutter-fix`.
- `...Portable.zip` runs without installation after extraction.

The binaries are not code-signed. Windows SmartScreen may display an
unknown-publisher warning. Verify downloads using `SHA256SUMS.txt`.
